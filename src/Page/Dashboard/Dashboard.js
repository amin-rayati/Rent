import React from 'react'
import user3 from '../../Assets/img/user-3.jpg'
import p1 from '../../Assets/img/p-1.png'
import p2 from '../../Assets/img/p-2.png'
import p3 from '../../Assets/img/p-3.png'
import p4 from '../../Assets/img/p-4.png'

import { ImLocation2 } from 'react-icons/im'
import {
  FaWallet,
  FaCreditCard,
  FaHome,
  FaCommentDots,
  FaHeart,
  FaTachometerAlt,
  FaUserAlt,
  FaBookmark,
  FaTasks,
  FaEnvelope,
  FaGift,
  FaPenAlt,
  FaUnlock,
  FaPowerOff,
  FaCog,
} from 'react-icons/fa'
import { BsCircleFill } from 'react-icons/bs'
const Dashboard = () => {
  return (
    <div id='main-wrapper'>
      <div class='page-title dashBoard' data-overlay='5'>
        <div class='container'>
          <div class='row'>
            <div class='col-lg-12 col-md-12 mt-5'>
              <div class='breadcrumbs-wrap'>
                <ol class='breadcrumb ' style={{ backgroundColor: 'none' }}>
                  <li class='breadcrumb-item active' aria-current='page'>
                    Dashboard
                  </li>
                </ol>
                <h2 class='breadcrumb-title'>Hello Adam, Welcome</h2>
              </div>
            </div>
          </div>
        </div>
      </div>
      <section class='gray pt-5 pb-5'>
        <div class='container-fluid'>
          <div class='row'>
            <div class='col-lg-9 col-md-8'>
              <div class='dashboard-body'>
                <div class='row'>
                  <div class='col-lg-12 col-md-12 col-sm-12'>
                    <h4>
                      Your Current Package:{' '}
                      <span class='pc-title theme-cl'>Gold Package</span>
                    </h4>
                  </div>
                </div>

                <div class='row'>
                  <div class='col-lg-4 col-md-6 col-sm-12'>
                    <div class='dashboard_stats_wrap widget-1'>
                      <div class='dashboard_stats_wrap_content'>
                        <h4>607</h4> <span>Listed Property</span>
                      </div>
                      <div class='dashboard_stats_wrap-icon'>
                        <ImLocation2 />
                      </div>
                    </div>
                  </div>

                  <div class='col-lg-4 col-md-6 col-sm-12'>
                    <div class='dashboard_stats_wrap widget-3'>
                      <div class='dashboard_stats_wrap_content'>
                        <h4>$10,540</h4> <span>Earned This Week</span>
                      </div>
                      <div class='dashboard_stats_wrap-icon'>
                        <FaWallet />
                      </div>
                    </div>
                  </div>

                  <div class='col-lg-4 col-md-6 col-sm-12'>
                    <div class='dashboard_stats_wrap widget-2'>
                      <div class='dashboard_stats_wrap_content'>
                        <h4>5,40580</h4> <span>Earned This Month</span>
                      </div>
                      <div class='dashboard_stats_wrap-icon'>
                        <FaCreditCard />
                      </div>
                    </div>
                  </div>
                </div>
                {/* <!--  row --> */}

                <div class='row'>
                  <div class='col-lg-8 col-md-7 col-sm-12'>
                    <div class='card'>
                      <div class='card-header'>
                        <h4 class='mb-0'>Extra Area Chart</h4>
                      </div>
                      <div class='card-body'>
                        <ul class='list-inline text-center m-t-40'>
                          <li>
                            <h5>
                              <BsCircleFill className='m-r-5 text-warning' />
                              Website A
                            </h5>
                          </li>
                          <li>
                            <h5>
                              <BsCircleFill className='m-r-5 text-danger' />
                              Website B
                            </h5>
                          </li>
                          <li>
                            <h5>
                              <BsCircleFill className='m-r-5 text-success' />
                              Website C
                            </h5>
                          </li>
                        </ul>
                        <div
                          class='chart'
                          id='extra-area-chart'
                          style={{ height: '300px' }}
                        ></div>
                      </div>
                    </div>
                  </div>

                  <div class='col-lg-4 col-md-5 col-sm-12'>
                    <div class='card'>
                      <div class='card-header'>
                        <h6>Notifications</h6>
                      </div>
                      <div class='ground-list ground-list-hove'>
                        <div class='ground ground-single-list'>
                          <a href='dashboard.html#'>
                            <div class='btn-circle-40 theme-cl theme-bg-light'>
                              <FaHome />
                            </div>
                          </a>

                          <div class='ground-content'>
                            <h6>
                              <a href='dashboard.html#'>
                                Your listing <strong>Azreal Modern</strong> has
                                been Verified!.
                              </a>
                            </h6>
                            <span class='small'>Just Now</span>
                          </div>
                        </div>

                        <div class='ground ground-single-list'>
                          <a href='dashboard.html#'>
                            <div class='btn-circle-40 theme-cl theme-bg-light'>
                              <FaCommentDots />
                            </div>
                          </a>

                          <div class='ground-content'>
                            <h6>
                              <a href='dashboard.html#'>
                                Litha Lynes left a review on{' '}
                                <strong>Renovated Apartment</strong>
                              </a>
                            </h6>
                            <span class='small'>20 min ago</span>
                          </div>
                        </div>

                        <div class='ground ground-single-list'>
                          <a href='dashboard.html#'>
                            <div class='btn-circle-40 theme-cl theme-bg-light'>
                              <FaHeart />
                            </div>
                          </a>

                          <div class='ground-content'>
                            <h6>
                              <a href='dashboard.html#'>
                                Someone bookmark your View listing!
                                <strong>Sargun Villa Bay</strong>
                              </a>
                            </h6>
                            <span class='small'>1 day ago</span>
                          </div>
                        </div>

                        <div class='ground ground-single-list'>
                          <a href='dashboard.html#'>
                            <div class='btn-circle-40 theme-cl theme-bg-light'>
                              <FaHome />
                            </div>
                          </a>

                          <div class='ground-content'>
                            <h6>
                              <a href='dashboard.html#'>
                                Your listing <strong>Modern Family Home</strong>{' '}
                                has been Verified!.
                              </a>
                            </h6>
                            <span class='small'>10 days ago</span>
                          </div>
                        </div>

                        <div class='ground ground-single-list'>
                          <a href='dashboard.html#'>
                            <div class='btn-circle-40 theme-cl theme-bg-light'>
                              <FaCommentDots />
                            </div>
                          </a>

                          <div class='ground-content'>
                            <h6>
                              <a href='dashboard.html#'>
                                Adam Brown left a review on{' '}
                                <strong>Renovated Apartment</strong>
                              </a>
                            </h6>
                            <span class='small'>Just Now</span>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                {/* <!-- row --> */}

                <div class='row'>
                  <div class='col-lg-12 col-md-12'>
                    <div class='card'>
                      <div class='card-header'>
                        <h4 class='mb-0'>Order Status</h4>
                      </div>
                      <div class='card-body p-0'>
                        <div class='table-responsive'>
                          <table class='table table-lg table-hover'>
                            <thead>
                              <tr>
                                <th>Name</th>
                                <th>Product ID</th>
                                <th>Status</th>
                                <th>Price</th>
                                <th>Date Created</th>
                              </tr>
                            </thead>

                            <tbody>
                              <tr>
                                <td>
                                  <a href='dashboard.html#'>
                                    <img
                                      src={p1}
                                      class='avatar avatar-30 mr-2'
                                      alt='Avatar'
                                    />
                                    Luxury House
                                  </a>
                                </td>
                                <td>#258475</td>
                                <td>
                                  <div class='label text-success bg-success-light'>
                                    Paid
                                  </div>
                                </td>
                                <td>$ 310</td>
                                <td>04/10/2013</td>
                              </tr>

                              <tr>
                                <td>
                                  <a href='dashboard.html#'>
                                    <img
                                      src={p2}
                                      class='avatar avatar-30 mr-2'
                                      alt='Avatar'
                                    />
                                    Sargun Apartment
                                  </a>
                                </td>
                                <td>#249578</td>
                                <td>
                                  <div class='label text-warning bg-warning-light'>
                                    Pending
                                  </div>
                                </td>
                                <td>$ 584.14</td>
                                <td>05/08/2014</td>
                              </tr>

                              <tr>
                                <td>
                                  <a href='dashboard.html#'>
                                    <img
                                      src={p3}
                                      class='avatar avatar-30 mr-2'
                                      alt='Avatar'
                                    />
                                    Preet Silver City
                                  </a>
                                </td>
                                <td>#248712</td>
                                <td>
                                  <div class='label text-danger bg-danger-light'>
                                    Cancel
                                  </div>
                                </td>
                                <td>$ 710.5</td>
                                <td>11/05/2015</td>
                              </tr>

                              <tr>
                                <td>
                                  <a href='dashboard.html#'>
                                    <img
                                      src={p4}
                                      class='avatar avatar-30 mr-2'
                                      alt='Avatar'
                                    />
                                    Mount See Villa
                                  </a>
                                </td>
                                <td>#287246</td>
                                <td>
                                  <div class='label text-success bg-success-light'>
                                    Paid
                                  </div>
                                </td>
                                <td>$ 482.70</td>
                                <td>06/09/2016</td>
                              </tr>
                            </tbody>
                          </table>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                {/* <!-- row --> */}
              </div>
            </div>

            <div class='col-lg-3 col-md-4'>
              <div class='property_dashboard_navbar'>
                <div class='dash_user_avater'>
                  <img src={user3} class='img-fluid avater' alt='' />
                  <h4>John S. Hopkinson</h4>
                  <span>Canada USA</span>
                </div>

                <div class='dash_user_menues'>
                  <ul>
                    <li class='active'>
                      <a href='dashboard.html'>
                        <FaTachometerAlt />
                        Dashboard
                        <span class='notti_coun style-1'>4</span>
                      </a>
                    </li>
                    <li>
                      <a href='my-profile.html'>
                        <FaUserAlt />
                        My Profile
                      </a>
                    </li>
                    <li>
                      <a href='bookmark-list.html'>
                        <FaBookmark />
                        Saved Property
                        <span class='notti_coun style-2'>7</span>
                      </a>
                    </li>
                    <li>
                      <a href='my-property.html'>
                        <FaTasks />
                        My Properties
                      </a>
                    </li>
                    <li>
                      <a href='messages.html'>
                        <FaEnvelope />
                        Messages
                        <span class='notti_coun style-3'>3</span>
                      </a>
                    </li>
                    <li>
                      <a href='choose-package.html'>
                        <FaGift />
                        Choose Package
                        <span class='expiration'>10 days left</span>
                      </a>
                    </li>
                    <li>
                      <a href='submit-property-dashboard.html'>
                        <FaPenAlt />
                        Submit New Property
                      </a>
                    </li>
                    <li>
                      <a href='change-password.html'>
                        <FaUnlock />
                        Change Password
                      </a>
                    </li>
                  </ul>
                </div>

                <div class='dash_user_footer'>
                  <ul>
                    <li>
                      <a href='dashboard.html#'>
                        <FaPowerOff />
                      </a>
                    </li>
                    <li>
                      <a href='dashboard.html#'>
                        <FaCommentDots />
                      </a>
                    </li>
                    <li>
                      <a href='dashboard.html#'>
                        <FaCog />
                      </a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>
    </div>
  )
}

export default Dashboard
